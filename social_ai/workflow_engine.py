...
def run_ai_node(node: NodeDef, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        ...
        system_parts.append(
            "أنت موظف مبيعات حقيقي يتحدث مع الزبون في تيليجرام بشكل طبيعي جداً، وليس بوتاً رسمياً. "
            "استخدم عربية بشرية لطيفة (فصحى مبسطة أو عامية مهنية حسب أسلوب الزبون). "
            "إذا وُجد سجل محادثة أدناه فاعتمد عليه للتماسك: لا تتناقض مع ما سبق، ولا تكرّر نفس المقدمة أو نفس الجملة الختامية في كل رد. "
            "إذا عاد الزبون لمنتج أو طلب سبق ذكره، أكمل من آخر نقطة مباشرة وكأنك تتابع الحديث معه فعلاً. "
            "للأسئلة البسيطة: جاوب مباشرة بجملة أو جملتين فقط. "
    ...
    if context.get("message_text") and str(context.get("knowledge") or "").strip():
        system_parts.append(
            "أنت مندوب مبيعات ودود: قدّم إجابة بشرية ومقنعة تعتمد فقط على معلومات الكتالوج أدناه؛ "
            "اذكر السعر والمخزون إن وُجدت بالكتالوج، واشرح الفائدة باختصار، وادعُ بلطف لإتمام الطلب فقط إذا كان الزبون قريباً من الشراء. "
    ...
    if task == "reply_comment" and conversation_history and context.get("chat_id"):
        ...
        system_parts.append(
            "لا تكرّر السعر أو المواصفات حرفياً إذا سبق ذكرها قبل قليل، إلا إذا طلب الزبون إعادة التوضيح. "
            "تكلم بأسلوب بشري مختصر، لا بأس أن يكون الرد بسيطاً ومباشراً."
        )
...
def _format_telegram_product_detail_addon(name: str, price: str, url: str) -> str:
    ...
    if price_raw:
        ...
        lines.append(f"السعر: {price_text} د.ع")
    u = str(url or "").strip()
    if u:
        lines.append(u)
...
def _append_product_share_links_if_needed(message: str, context: Dict[str, Any]) -> str:
    ...
    if _is_more_details_request_message(user_message) and url:
        extra_lines.append(_format_telegram_product_detail_addon(name, price, url))
